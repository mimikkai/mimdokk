...
Вы можете запустить утилиту сразу, не устанавливая её в проект:

```bash
npx @mimikkai/mimdokk inspect template.docx
```

> **Важно**: Чтобы `npx` мог найти пакет, у вас должен быть настроен доступ к GitHub Packages. Создайте файл `.npmrc` (в текущей папке или `~/.npmrc`) с содержимым:
> ```ini
